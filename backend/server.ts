...
async function startServer() {
  ...
  // Add production Vercel domain if provided via APP_URL
  if (process.env.APP_URL) {
    allowedOrigins.push(process.env.APP_URL);
  }

  // Add Vercel preview URLs automatically
  ...
  // In a production app, replace this with Redis for persistence across restarts
  ...
  async function getSpotifyTrackMetadata(
    trackId: string
  ...
  function detectPlatform(
    url: string
  ): "spotify" | "soundcloud" | "youtube" | "generic" | null {
  ...
  app.get("/api/youtube/search", async (req, res) => {
    ...
    if (!apiKey) {
      ...
      return res.status(500).json({
        error:
          "YouTube API key not configured. Add YOUTUBE_API_KEY to your environment.",
      });
    }
    ...
    try {
      ...
      if (platform === "spotify") {
        ...
        if (!metadata)
          return res
            .status(400)
            .json({ error: "Could not fetch Spotify track metadata" });
    ...
    upload.single("file")(req, res, (err: any) => {
      if (err instanceof multer.MulterError) {
        if (err.code === "LIMIT_FILE_SIZE") {
          return res
            .status(400)
            .json({ error: "File is too large. Max size is 15MB." });
        }
        return res
          .status(400)
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("party:mute", ({ code, listener_id, muted }) => {
      ...
      if (party && party.hostId === socket.id) {
        const listener = party.listeners.find(
          (l: any) => l.id === listener_id
        );
        if (listener) {
    ...
    socket.on("party:ban", ({ code, listener_id }) => {
      const party = parties.get(code);
      if (party && party.hostId === socket.id && listener_id !== socket.id) {
        const targetListener = party.listeners.find(
          (l: any) => l.id === listener_id
        );
        if (targetListener) {
    ...
    socket.on("sync:play", ({ code, track_id, position_ms }) => {
      ...
      if (party && party.hostId === socket.id) {
        let track =
          party.currentTrack?.id === track_id
            ? party.currentTrack
            : party.queue.find((t: any) => t.id === track_id);
    ...
    socket.on("sync:pause", ({ code }) => {
      ...
      if (party && party.hostId === socket.id) {
        ...
        const elapsed = party.playbackState.playing
          ? Math.max(0, now - party.playbackState.timestamp)
          : 0;
        party.playbackState = {
          playing: false,
    ...
    socket.on("disconnect", () => {
      ...
      for (const [code, party] of parties.entries()) {
        const index = party.listeners.findIndex(
          (l: any) => l.id === socket.id
        );
        if (index !== -1) {
  ...
  httpServer.listen(PORT, "0.0.0.0", () => {
    console.log(`SyncWave backend running on http://0.0.0.0:${PORT}`);
  });
}
